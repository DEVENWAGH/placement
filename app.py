    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        if account:
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s)
            """, (username, _hashed_password, email, role, fullname))
...
def apply_job(job_id):
    ...
    cur = conn.cursor(dictionary=True, buffered=True)
    cur.execute("""
        INSERT INTO applications (student_id, job_id, applied_date)
        VALUES (%s, %s, CURDATE())
...
def post_job():
    ...
    if request.method == 'POST':
        ...
        # Get form data and insert into Job table
        cur = conn.cursor(dictionary=True, buffered=True)
        cur.execute("""
            INSERT INTO Job (job_Id, company, position, recruiter_id, posted_date)
            VALUES (%s, %s, %s, %s, CURDATE())
        """, (job_id, company, position, session['username']))
        conn.commit()
...
def submit_feedback():
    ...
    cur = conn.cursor(dictionary=True, buffered=True)
    cur.execute("""
        INSERT INTO feedback (faculty_id, student_id, message)
        VALUES (%s, %s, %s)
    """, (session['username'], student_id, message))
    conn.commit()
